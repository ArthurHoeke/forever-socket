...
var help = [
  ...
  '  link <socket_url> <secret_key> [server_name] Sets the specified forever config',
...
//
// ### function link (socketUrl, secretKey, serverName)
// #### @socketUrl {string} URL of the socket
// #### @secretKey {string} Secret key for authentication
// #### @serverName {string} Name of the server
// Sets the specified `socketUrl`, `secretKey`, and `serverName` in the
// user config.
//
app.cmd(/link ([^ ]+) ([^ ]+) ([^ ]+)/, cli.link = function (socketUrl, secretKey, serverName) {
  updateConfig(function () {
    forever.log.info('Setting forever config: socket_url, secret_key, server_name');
    forever.config.set('socket_url', socketUrl);
    forever.config.set('secret_key', secretKey);
    forever.config.set('server_name', serverName);
  });
});
