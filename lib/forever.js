...
forever.start = function (script, options) {
  ...
  //TODO: pass socketio config data to monitor. in monitor itself add socket responses
  const socket_url = forever.config.get('socket_url');
  const secret_key = forever.config.get('secret_key');
  const server_name = forever.config.get('server_name');

  if(socket_url) {
    socket = io(socket_url, { query: { secret_key: secret_key, server_name: server_name } })
  }
